...
class UserIDIntentHandler(AbstractRequestHandler):
    """Handler for Existing User ID Info """
    def can_handle(self, handler_input):
...
class ScheduleIntentHandler(AbstractRequestHandler):
    """Handler for Schedule Appointment Intent."""
    def can_handle(self, handler_input):
        # type: (HandlerInput) -> bool
    ...
    def handle(self, handler_input):
        ...
        return (
            handler_input.response_builder
                .speak(f"The available doctors in the {department} department are {doctors_str}. Please select a doctor for scheduling.")
                .response
        )
        
class doctorScheduleHandler(AbstractRequestHandler):
    """Handler for Scheduling Appointment with a particular doctor"""
    def can_handle(self, handler_input):
...
def send_email(email, subject,body):
    ses_client = boto3.client('ses',region_name="us-east-1")
    sender_email = "sangameshwarp1105@gmail.com" 
    
    response = ses_client.send_email(
        Source=sender_email,
        Destination={'ToAddresses': [email]},
        #Destination={'ToAddresses': ["sangameshwarp1105@gmail.com"]},
        Message={
